...
namespace Finary.Core.Repositories
{
    public class WithdrawalRepository : GenericRepository<Withdrawal>
    {
        ...
        public WithdrawalRepository(FinaryEntities db, bool disableLazyLoading = false) : base(db,disableLazyLoading) { }
        ...
        {
            return (from w in db.Withdrawal where w.xStatus==(byte)WithdrawalStatusType.WaitingForAccept select w.xID).Count();
        }
        public int GetTotalInPaymentList()
        {
            return (from w in db.Withdrawal where w.xStatus == (byte)WithdrawalStatusType.InPaymentList select w.xID).Count();
        }
        ...
        public Withdrawal GetLastByUserID(long userID)
        {
            return (from w in db.Withdrawal where w.xUserID==userID orderby w.xDate descending select w).Take(1).SingleOrDefault();
        }

        public long waitingCashAmount()
        {
            var records = (from w in db.Withdrawal where (w.xStatus == (byte)WithdrawalStatusType.InPaymentList || w.xStatus == (byte)WithdrawalStatusType.WaitingForAccept) && w.UserBankAccount != null select w.xAmount);
            if(records.Count()==0)
            {
                return 0;
            }
