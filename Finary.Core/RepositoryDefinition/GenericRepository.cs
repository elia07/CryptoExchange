...
{
    public abstract class GenericRepository<T> : IDisposable, IGenericRepositoryAsync<T> where T : class
    {
        bool savePermission = true;
        public FinaryEntities db;
        public DbSet<T> dbSet;

        public GenericRepository(FinaryEntities dbInstance = null, bool disableLazyLoading = false)
        {
            if (dbInstance == null)
            {
                this.db = new FinaryEntities();
            }
            else
            {
        ...
        public async Task<T> GetByID(object id, bool NoTracking = false)
        {
            if (NoTracking)
            {
        ...
        public async Task<T> GetByID(long id, bool NoTracking = false)
        {
            if (NoTracking)
            {
                var entity = await dbSet.FindAsync(id);
        ...
        public async Task<long> Count(Expression<Func<T, bool>> predicate)
        {
            try
            {
                ...
                {
                    return await dbSet.AsExpandable().Where(predicate).CountAsync();
                }
